...
if (existsSync('deployment.json')) {
  ...
  ultrahonkVerifierId = deploymentInfo?.ultrahonkVerifierId || ultrahonkVerifierId;
} else {
  const env = await readEnvFile('.env');
  for (const contract of contracts) {
    contractIds[contract.packageName] = getEnvValue(env, `VITE_${contract.envKey}_CONTRACT_ID`);
  }
  ultrahonkVerifierId = getEnvValue(env, 'VITE_ULTRAHONK_VERIFIER_CONTRACT_ID', ultrahonkVerifierId);
